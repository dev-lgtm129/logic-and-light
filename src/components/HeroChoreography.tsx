...
export const HeroChoreography: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // EXACT ORIGINAL DELIBERATE TIMING (Phase 1: 0-1.48s, Phase 2: 1.48s-2.8s)
  ...
  const allPhotos = getHeroCollisionPhotos();

  // =========================================================================
  // 1. DESKTOP-ONLY 100% UNTOUCHED ORIGINAL PATH (Exact original 6 photos, 250px radius, 950px perspective & 74° 3D cuboid)
  // =========================================================================
  if (!isMobile) {
  ...
  // =========================================================================
  // 2. MOBILE-ONLY ISOLATED PATH (Lightweight 3 photos, 150px radius, 600px perspective & GPU friendly transforms)
  // =========================================================================
  const mobilePhotos = allPhotos.slice(0, 3);
  const mobileRadius = 150;

  const mobileWallTransforms = [
    { transform: 'translate3d(-50%, -170px, 0) scale(1.05)' },
    { transform: 'translate3d(-50%, 130px, 0) scale(1.05)' },
  ...
  return (
    <div className="relative w-full min-h-[92dvh] flex flex-col justify-center items-center px-6 pt-24 pb-12 overflow-hidden select-none bg-black">
      ...
      >
        <div
          className={`absolute top-1/2 left-1/2 w-[1px] h-[1px] pointer-events-none transform-gpu ${
            phase === 'orbit' ? 'animate-orbit-ring' : ''
          }`}
          style={{ willChange: phase === 'orbit' ? 'transform' : 'auto' }}
        >
          {mobilePhotos.map((src, index) => {
            const baseAngle = (index / 3) * Math.PI * 2 - Math.PI / 2;
            const orbitX = Math.cos(baseAngle) * mobileRadius;
            const orbitY = Math.sin(baseAngle) * mobileRadius;
            const wallTransform = mobileWallTransforms[index].transform;
            ...
            let currentStyle: React.CSSProperties = {
              ...
              top: '0px',
              left: '0px',
              willChange: phase !== 'settled' ? 'transform, opacity' : 'auto',
            };
            ...
            return (
              ...
              >
                <div
                  className={`w-full h-full rounded-xl overflow-hidden border border-white/20 shadow-2xl bg-zinc-900 transform-gpu ${
                    phase === 'orbit' ? 'animate-orbit-counter' : ''
                  }`}
                  style={{ willChange: phase === 'orbit' ? 'transform' : 'auto' }}
                >
